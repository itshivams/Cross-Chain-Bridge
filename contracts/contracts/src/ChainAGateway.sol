...
contract ChainAGateway is IChainGateway, Ownable, ReentrancyGuard {
    ...
    constructor(address _tokenA, address _trustedSigner) Ownable(msg.sender) {
    ...
    function lock(uint256 amount, uint256 nonce, bytes calldata signature)
        external
        override
        nonReentrant
    {
    ...
    function unlock(uint256 amount, bytes32[] calldata merkleProof)
        external
        override
        onlyOwner
        nonReentrant
